...
이름이 통용되는 문맥을 넓힐 때 생기는 단점은 할 일이 늘어난다는 것입니다. 많이 늘어날 것 같다면 `임시 변수를 질의 함수로 바꾸기`를 적용할 수 있을 때까지 일단 놔둡니다. 간단히 처리할 수 있다면 즉시 넓혀서 다른 코드에서도 사용할 수 있게 합니다. 가령 클래스 안의 코드를 다룰 때는 [`함수 추출하기`](https://github.com/Esoolgnah/Summary_of_Refactoring_2nd_Edition/blob/main/Notes/06_기본적인_리팩터링/06_01_함수_추출하기.md)를 아주 쉽게 적용할 수 있습니다.
...
1️⃣ 간단한 코드지만 더 쉽게 만들 수 있습니다. 먼저 기본 가격은 상품 가격(itemPrice)에 수량(quantity)을 곱한 값임을 파악해내야 합니다.
